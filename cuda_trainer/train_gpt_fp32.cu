...
void gpt2_build_from_checkpoint(GPT *model, const char* checkpoint_path) {
...
void gpt2_forward(GPT *model, int* inputs, int* targets, int B, int T) {
...
void gpt2_update(GPT *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, int t) {
...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step <= train_num_batches; step++) {
        ...
        if (step > 0 && step % sample_every == 0 || last_step) {
            // fill up gen_tokens with the GPT_EOT, which kicks off the generation
            for(int i = 0; i < B * T; ++i) {
